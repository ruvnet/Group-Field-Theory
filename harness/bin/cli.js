...
// Generated by metaharness — the `group-field-theory-harness` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx group-field-theory-harness`
...
// "bin" + "files" fields in package.json), so `npx group-field-theory-harness` works the moment
...
/** `group-field-theory-harness init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `group-field-theory-harness doctor` — verify the install end-to-end (kernel + host resolve). */
...
async function loadBuilt(command) {
  try {
    await import(`../dist/${command}.js`);
    return 0;
  } catch (error) {
    if (error?.code === 'ERR_MODULE_NOT_FOUND') {
      console.error(`Build required. Run \`npm run build\` before \`${HARNESS_NAME} ${command}\`.`);
      return 2;
    }
    throw error;
  }
...
export async function run(argv) {
  ...
  switch (cmd) {
    ...
    case 'doctor':
      return doctor();
    case 'benchmark':
      return loadBuilt('benchmark');
    case 'optimize':
      return loadBuilt('optimize');
    case '--version':
    case '-v': {
      const kernel = await loadKernel();
